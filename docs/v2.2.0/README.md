...
For this analysis, the phosphatase-substrate interactions in the [DEPOD database](http://depod.bioss.uni-freiburg.de/) are used. Overall, ``1531`` (``XXX``) new interactions are added to the Rokai network data for ``human`` (``mouse``) species. These interactions are between ``77``(``YY``) phosphatase and ``478`` (``ZZZ``) phosphosites.
